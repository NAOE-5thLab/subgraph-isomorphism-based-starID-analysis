...
def uniform_spherical_vector(n: int, np_random: np.random.Generator):
    u = np_random.uniform(size=n)
    v = np_random.uniform(size=n)
    z = -2 * u + 1
    x = np.sqrt(1 - z**2) * np.cos(2 * np.pi * v)
    y = np.sqrt(1 - z**2) * np.sin(2 * np.pi * v)
    b = np.concatenate([x[:, np.newaxis], y[:, np.newaxis], z[:, np.newaxis]], axis=1)
    if n == 1:
        b = b[0]
    return b


def limit_uniform_spherical_vector(
    sigma: float, n: int, np_random: np.random.Generator
):
    u = np_random.uniform(size=n)
    v = np_random.uniform(size=n)
...
# def normal_spherical_approximated_vector(
#     mu, sigma, n: int, np_random: np.random.Generator
# ):
#     eps = 1.0e-8
...
#     n_y = np.cross(n_z, mu)
#     #
#     k_y = np_random.normal(n) * sigma
#     k_z = np_random.normal(n) * sigma
