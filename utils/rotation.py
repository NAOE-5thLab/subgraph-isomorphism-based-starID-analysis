...
def rodrigues_rotation_matrix(theta, n):
    ...
    if r11.shape == ():
        ...
        r1 = np.concatenate(
            [
                r11[:, np.newaxis, np.newaxis],
                r12[:, np.newaxis, np.newaxis],
                r13[:, np.newaxis, np.newaxis],
            ],
            axis=2,
        )
        r2 = np.concatenate(
            [
                r21[:, np.newaxis, np.newaxis],
                r22[:, np.newaxis, np.newaxis],
                r23[:, np.newaxis, np.newaxis],
            ],
            axis=2,
        )
        r3 = np.concatenate(
            [
                r31[:, np.newaxis, np.newaxis],
                r32[:, np.newaxis, np.newaxis],
                r33[:, np.newaxis, np.newaxis],
            ],
            axis=2,
        )
...
def rotation_on_earth(vec, alpha, delta, A, get_basis=False):
    ...
    #
    e_x_alpha_delta_A = rodrigues_rotation_matrix(A, -e_x_alpha_delta) @ e_x_alpha_delta
    e_y_alpha_delta_A = rodrigues_rotation_matrix(A, -e_x_alpha_delta) @ e_y_alpha_delta
    e_z_alpha_delta_A = rodrigues_rotation_matrix(A, -e_x_alpha_delta) @ e_z_alpha_delta
    vec_alpha_delta_A = rodrigues_rotation_matrix(A, -e_x_alpha_delta) @ vec_alpha_delta
    if get_basis:
        info = {
            "e_x": e_x,
            "e_y": e_y,
