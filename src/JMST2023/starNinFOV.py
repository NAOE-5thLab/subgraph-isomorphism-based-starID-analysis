...
def sim():
    ...
    eq_vec = equatorial2vec(star_ctlg.get_RA(), star_ctlg.get_DE())

    ### Monte Carlo ###
    tan_FOV2 = np.tan(FOV/2.0)
    # loop
    log_file = open(f'{log_dir}/inFOV_{FOV_deg}_{Mv_max}.dat', 'w')
    for i in tqdm.tqdm(range(roopN)):
        ...
        # FOV condition
        cond_1 = eq_vec_R[:, :, 0] > 0.0
        temp = tan_FOV2*eq_vec_R[:, :, 0]
        cond_2 = np.abs(eq_vec_R[:, :, 1]) < temp
        cond_3 = np.abs(eq_vec_R[:, :, 2]) < temp
...
def hist_csv():
    ...
    for i, Mv_max in enumerate([3.5, 4.5, 5.5]):
        for j, FOV_deg in enumerate([20, 40, 60, 80]):
            print(f'FOV : {FOV_deg}, Mv : {Mv_max}')
            progress = 0
            counter = np.zeros(width_list[i]+1)
            for line in line_gene(f'{log_dir}/inFOV_{FOV_deg}_{Mv_max}.dat'):
                num = int(line)
                counter[num] += 1
                progress += 1
                if progress % 1e8==0:
                    print(f'progress : {progress}')
            with open(f"{log_dir}/inFOV_{FOV_deg}_{Mv_max}_dens.dat", mode='w') as f:
...
def plot_all():
    ...
    for i, Mv_max in enumerate([3.5, 4.5, 5.5]):
        ...
        for j, FOV_deg in enumerate([20, 40, 60, 80]):
            ...
            log_file = open(f'{log_dir}/inFOV_{FOV_deg}_{Mv_max}.dat', 'r')
        ...
        ax.legend()
        fig.savefig(f"{log_dir}/inFOV_Mv{Mv_max}_dens.pdf")
...
def prob_n(n=10):
    for i, Mv_max in enumerate([3.5, 4.5, 5.5]):
        for j, FOV_deg in enumerate([20, 40, 60, 80]):
            print(f'FOV : {FOV_deg}, Mv : {Mv_max}')
            print(f'loading data ...')
            load_file = open(f'{log_dir}/inFOV_{FOV_deg}_{Mv_max}.dat', 'r')
            num_list = load_file.readlines()
            nums = np.array([int(num) for num in num_list])
            load_file.close()
            #
            log_file = open(f'{log_dir}/inFOV_{FOV_deg}_{Mv_max}_prob_{n}.dat', 'w')
...
def plot_prob(n=10):
    ...
    for i, Mv_max in enumerate([3.5, 4.5, 5.5]):
        for j, FOV_deg in enumerate([20, 40, 60, 80]):
            print(f'FOV : {FOV_deg}, Mv : {Mv_max}')
            print(f'loading data ...')
            load_file = open(f'{log_dir}/inFOV_{FOV_deg}_{Mv_max}_prob_{n}.dat', 'r')
            alphas = []
            probs = []
...
def plot_starnum_prob(star_num):
    ...
    # for i, Mv_max in enumerate([3.5, 4.5, 5.5]):
    #     for j, FOV_deg in enumerate([20, 40, 60, 80]):
    #         print(f'Magnitude Limit = {Mv_max} [Mv], FOV = {FOV_deg} [deg.]')
    #         with open(f"{log_dir}/inFOV_{FOV_deg}_{Mv_max}_dens.dat", mode='r') as f:
    #             lines = f.readlines()
    #         pNs = np.zeros(len(lines))
    #         count_all = 0
    ...
    #             probs.append(prob)
    #         # 
    #         df[f'Mv{Mv_max}_FOV{FOV_deg}'] = probs
    ...
    for i, Mv_max in enumerate([3.5, 4.5, 5.5]):
        for j, FOV_deg in enumerate([20, 40, 60, 80]):
            probs = df[f'Mv{Mv_max}_FOV{FOV_deg}']
            ax.plot(
                alphas, probs,
                color=color_list[j],
                linestyle=ls_list[i],
                label=f'{Mv_max}, {FOV_deg}')
